...
int load_config(){
  ...
  //initialize sensors array :
  sensors=gmalloc(nb_sensors*sizeof(struct DEVICE));
  actuatorAlreadyUpdated=gmalloc(nb_sensors*sizeof(int));
  //Those arrays are used until the end of the app, therefore we don't care much
  //about freeing them.
  fclose(conf);
  return 0;
}
